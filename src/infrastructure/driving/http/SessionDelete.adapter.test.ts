/** biome-ignore-all lint/suspicious/noExplicitAny: it is expected in tests */
import type SessionDeleteUseCase from '@application/usecases/SessionDelete.usecase'
import { beforeEach, describe, expect, it, type Mock, vi } from 'vitest'
import SessionDelete from './SessionDelete.adapter'

describe('SessionDelete', () => {
	let mockUseCase: SessionDeleteUseCase
	let endpoint: import('@infrastructure/utilities/ExpressEndpoint.adapter').IEndpoint

	beforeEach(() => {
		mockUseCase = {
			execute: vi.fn().mockResolvedValue(undefined),
		} as unknown as SessionDeleteUseCase
		endpoint = SessionDelete(mockUseCase)
	})

	it('returns IEndpoint with correct method and path', () => {
		expect(endpoint.method).toBe('DELETE')
		expect(endpoint.path).toBe('/sessions/{id:string}')
	})

	it('returns 2 handlers from toHandlers()', () => {
		const handlers = endpoint.toHandlers()

		expect(Array.isArray(handlers)).toBe(true)
		expect(handlers.length).toBe(2)
		expect(typeof handlers[0]).toBe('function')
		expect(typeof handlers[1]).toBe('function')
	})

	it('validates id param is present', async () => {
		const handlers = endpoint.toHandlers()
		const validationMiddleware = handlers[0] as CallableFunction

		const mockReq = {
			params: {},
			body: {},
			query: {},
		} as any
		const mockRes = {
			status: vi.fn().mockReturnThis(),
			json: vi.fn(),
		} as any
		const next = vi.fn()

		validationMiddleware(mockReq, mockRes, next)

		expect(mockRes.status).toHaveBeenCalledWith(400)
		expect(mockRes.json).toHaveBeenCalled()
		expect(next).not.toHaveBeenCalled()
	})

	it('calls useCase.execute with parsed id', async () => {
		const handlers = endpoint.toHandlers()
		const dispatchMiddleware = handlers[1] as CallableFunction

		const mockReq = {
			params: { id: 'session-123' },
			body: {},
			query: {},
			on: vi.fn(),
		} as any
		const mockRes = {
			sendStatus: vi.fn(),
			status: vi.fn().mockReturnThis(),
			json: vi.fn(),
		} as any

		const next = vi.fn()
		const promise = dispatchMiddleware(mockReq, mockRes, next)
		await promise

		expect(mockUseCase.execute).toHaveBeenCalledWith('session-123')
	})

	it('returns 204 No Content for void handler', async () => {
		const handlers = endpoint.toHandlers()
		const dispatchMiddleware = handlers[1] as CallableFunction

		const mockReq = {
			params: { id: 'session-123' },
			body: {},
			query: {},
			on: vi.fn(),
		} as any
		const mockRes = {
			sendStatus: vi.fn(),
			status: vi.fn().mockReturnThis(),
			json: vi.fn(),
		} as any

		const next = vi.fn()
		const promise = dispatchMiddleware(mockReq, mockRes, next)
		await promise

		expect(mockRes.sendStatus).toHaveBeenCalledWith(204)
	})

	it('returns 500 when useCase throws', async () => {
		;(mockUseCase.execute as Mock).mockRejectedValue(new Error('Delete failed'))

		const handlers = endpoint.toHandlers()
		const dispatchMiddleware = handlers[1] as CallableFunction

		const mockReq = {
			params: { id: 'session-123' },
			body: {},
			query: {},
			on: vi.fn(),
		} as any
		const mockRes = {
			sendStatus: vi.fn(),
			status: vi.fn().mockReturnThis(),
			json: vi.fn(),
		} as any

		const next = vi.fn()
		const promise = dispatchMiddleware(mockReq, mockRes, next)
		await promise

		expect(mockRes.status).toHaveBeenCalledWith(500)
		expect(mockRes.json).toHaveBeenCalled()
	})
})
