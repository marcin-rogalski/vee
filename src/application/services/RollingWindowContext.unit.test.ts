...
const repo = { create: vi.fn(), get: vi.fn(), update: vi.fn().mockResolvedValue(undefined) };
...
describe("RollingWindowContext", () => {
	...
	it("history within budget is fully included", () => {
		...
		ctx.startTurn("hi");
		expect(ctx.entries).toContainEqual(expect.objectContaining({ content: "hello" }));
		expect(ctx.entries).toContainEqual(expect.objectContaining({ content: "world" }));
	});
	...
	it("oldest history is dropped when over budget", () => {
		...
		ctx.startTurn("hi");
		expect(ctx.entries).toContainEqual(expect.objectContaining({ content: "world" }));
		expect(ctx.entries).not.toContainEqual(expect.objectContaining({ content: "hello" }));
	});
	...
	it("commitTurn moves user and currentTurn into history for next turn", async () => {
		...
		ctx.commitTurn();
		ctx.startTurn("next");
		expect(ctx.entries).toContainEqual(expect.objectContaining({ content: "hey" }));
		expect(ctx.entries).toContainEqual(expect.objectContaining({ content: "hi" }));
	});
});
