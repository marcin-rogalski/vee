...
import ChatMessageUseCase from "./ChatMessage.uscase";
...
function fakeContext(): ChatContextPort & { pushed: ChatEntry[] } {
	...
	return {
		...
		push: vi.fn(async (...entries: ChatEntry[]) => { pushed.push(...entries); }),
		startTurn: vi.fn(),
		commitTurn: vi.fn(),
		pushed,
	};
...
function fakeToolManager(result = "ok") {
	return { getTools: vi.fn().mockResolvedValue([]), executeTool: vi.fn().mockResolvedValue(result) };
}

describe("ChatMessageUseCase", () => {
	it("yields token events and pushes assistant message", async () => {
		const ctx = fakeContext();
		const useCase = new ChatMessageUseCase(fakeContextManager(ctx) as any, fakeToolManager() as any);
		...
		const events = [];
		for await (const e of useCase.execute("s1", "hi", model as any)) events.push(e);

		expect(events).toContainEqual({ type: "token", data: { content: "hel" } });
		expect(ctx.push).toHaveBeenCalledWith(expect.objectContaining({ author: "assistant", content: "hello" }));
	...
	it("executes tool call and loops until done", async () => {
		...
		const useCase = new ChatMessageUseCase(fakeContextManager(ctx) as any, toolManager as any);
		const model = fakeModel([
			[{ type: "tool-call", data: { id: "tc1", name: "myTool", arguments: {} } }, { type: "done" }],
		...
		expect(events).toContainEqual({ type: "tool-response", data: { toolCallId: "tc1", result: "tool-result" } });
